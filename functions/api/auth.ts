...
async function checkLoginBlock(ip: string): Promise<{ blocked: boolean; remaining: number; attemptCount: number }> {
  ...
  const record = rows[0] as { attempt_count: number; locked_until: string | null };
...
async function recordFailedLogin(ip: string): Promise<{ attemptCount: number; lockedUntil: string | null; remaining: number }> {
  ...
  if (Array.isArray(rows) && rows.length > 0) {
    const rec = rows[0] as { id: string; attempt_count: number; locked_until: string | null };
...
async function recordSuccessLogin(ip: string): Promise<void> {
  const rows = await sf("GET", `login_attempts?ip=eq.${encodeURIComponent(ip)}&order=id.desc&limit=1`);
  if (Array.isArray(rows) && rows.length > 0) {
    const rec = rows[0] as { id: string; locked_until: string | null };
...
export async function onRequestPost({ request }: { request: Request }) {
  ...
  try {
    ...
    if (body.action === "admin_generate") {
      const { secret, count, plan } = body as { secret: string; count?: number; plan?: string };
    ...
    if (body.action === "admin_generate_plan_keys") {
      const { secret, plan, count } = body as { secret: string; plan: string; count?: number };
      if (secret !== ADMIN_SECRET) return makeRes(403, { error: "无权" });
    ...
    if (body.action === "signup") {
      const { username, password, inviteCode } = body as { username: string; password: string; inviteCode: string };
      ...
      const userId = Array.isArray(user) ? (user as Array<{id?:string}>)[0]?.id : (user as { id?: string }).id;
    ...
    if (body.action === "login") {
      ...
      const u = users[0] as { id: string; username: string; password_hash: string; member_expires_at: string | null };
    ...
    if (body.action === "change_password") {
      const { userId, oldPassword, newPassword } = body as { userId: string; oldPassword: string; newPassword: string };
      ...
      const u = users[0] as { password_hash: string };
    ...
    if (body.action === "set_daily_count") {
      const { userId, dailyCount } = body as { userId: string; dailyCount: number };
    ...
    if (body.action === "activate_plan") {
      const { userId, keyCode } = body as { userId: string; keyCode: string };
      ...
      const u = (Array.isArray(users) && users.length > 0) ? users[0] as { member_expires_at: string | null } : { member_expires_at: null };
